...
ggplot(data = joined, aes( x= state, y = award_amount_2010.2011, colour = state)) +
     geom_point()
...
ggplot(data = bar_data, aes( x = state, y = total, fill = state)) +
...
```{r echo = FALSE, warning=FALSE, fig.width=10}
ggplot(data = joined, aes(x=joined$award_amount_2010.2011)) +
  geom_histogram() +
     xlab("Grant Amount ($)") +
     theme_hc()
```
